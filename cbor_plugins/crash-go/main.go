// main.go - crash service using cbor plugin system
...
var log = logging.MustGetLogger("crash")
...
func main() {
	...
	// Log to a file.
	logFile := path.Join(logDir, fmt.Sprintf("crash.%d.log", os.Getpid()))
	...
	// start service
	server := http.Server{}
	tmpDir, err := ioutil.TempDir("", "crash_server")
	if err != nil {
		panic(err)
	}
	socketFile := filepath.Join(tmpDir, fmt.Sprintf("%d.crash.socket", os.Getpid()))
